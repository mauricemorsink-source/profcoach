...
const [roleModal, setRoleModal] = useState<User | null>(null);
  ...
  type PredConfig = {
    ...
    totalGoalsMin: number | null; totalGoalsMax: number | null;
    topScorerPoints: number; assistKoningPoints: number; yellowCardsPoints: number; totalGoalsPoints: number;
  ...
  const [predConfigForm, setPredConfigForm] = useState({
    ...
    topScorerPoints: "5", assistKoningPoints: "5", yellowCardsPoints: "5", totalGoalsPoints: "5",
  ...
  type PredPreview = { config: PredConfig; total: number; topScorerCount: number | null; assistKoningCount: number | null; yellowCardsCount: number | null; totalGoalsCount: number | null };
  ...
  async function openPredPreview() {
    setLoadingPredPreview(true); setShowPredPreview(true); setPredPreview(null);
  ...
  async function doPublishMoment(momentId: string, excludedPerformances: { playerId: string; matchId: string }[]) {
    setPublishingMomentId(momentId); setPointsMsg(null);
  ...
  async function publishMoment(id: string) {
    setCheckingConflicts(true); setPointsMsg(null);
                          ...
                          <div key={pm.id} className={`rounded-xl border p-3 ${isPast ? "border-amber-500/30 bg-amber-900/10" : "border-slate-700 bg-slate-800/50"}`}>
                            ...
                            <div className="flex gap-1.5">
                              <button
                                ...
                                disabled={publishingMomentId === pm.id || checkingConflicts || approvedInMoment.length === 0}
                              ...
                              >
                                {publishingMomentId === pm.id ? "Verwerken..." : checkingConflicts ? "Controleren..." : `Publiceer (${approvedInMoment.length})`}
                              </button>
                              <button
        ...
        {activeTab === "bonusvragen" && (
          <section className="bg-slate-900 neon-border rounded-2xl p-6 space-y-5">
            ...
            {loadingPredConfig ? <p className="text-slate-500 text-sm">Laden...</p> : (<>
              ...
              {/* Helper component: player picker inline */}
              {/* Vraag 1: Topscorer */}
              <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input type="number" min="0" value={predConfigForm.topScorerPoints} onChange={(e) => setPredConfigForm(f => ({ ...f, topScorerPoints: e.target.value }))}
                ...
                {predActiveField === "topscorer" ? (
                    ...
                    <div className="max-h-44 overflow-y-auto rounded-xl border border-slate-700 bg-slate-900/60">
                      {players.filter(p => !predPlayerSearch.trim() || p.name.toLowerCase().includes(predPlayerSearch.toLowerCase())).slice(0, 30).map(p => (
                        <button key={p.id} onClick={() => { setPredConfigForm(f => ({ ...f, topScorerId: p.id })); setPredActiveField(null); setPredPlayerSearch(""); }}
                          ...
                          <span>{p.name}</span><span className="text-slate-500 text-xs">{TEAM_LABEL[p.clubTeam] ?? p.clubTeam}</span>
                        </button>
                      ))}
                    </div>
                  ...
                  <button onClick={() => { setPredActiveField("topscorer"); setPredPlayerSearch(""); }}
                    className={`w-full text-left px-3 py-2.5 rounded-xl border text-sm transition-colors ${predConfigForm.topScorerId ? "border-cyan-500/40 bg-cyan-500/10 text-white" : "border-slate-600 bg-slate-900/50 text-slate-400 hover:border-slate-500"}`}>
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <label className="text-xs text-slate-500">Punten:</label>
                    <input type="number" min="0" value={predConfigForm.assistKoningPoints} onChange={(e) => setPredConfigForm(f => ({ ...f, assistKoningPoints: e.target.value }))}
                      className="w-16 bg-slate-900 border border-slate-700 text-white rounded-lg px-2 py-1 text-xs text-center focus:outline-none focus:ring-1 focus:ring-cyan-500/40" />
                  </div>
                </div>
                {predActiveField === "assistkoning" ? (
                  ...
                  <button onClick={() => { setPredActiveField("assistkoning"); setPredPlayerSearch(""); }}
                    className={`w-full text-left px-3 py-2.5 rounded-xl border text-sm transition-colors ${predConfigForm.assistKoningId ? "border-cyan-500/40 bg-cyan-500/10 text-white" : "border-slate-600 bg-slate-900/50 text-slate-400 hover:border-slate-500"}`}>
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <label className="text-xs text-slate-500">Punten:</label>
                    <input type="number" min="0" value={predConfigForm.yellowCardsPoints} onChange={(e) => setPredConfigForm(f => ({ ...f, yellowCardsPoints: e.target.value }))}
                      className="w-16 bg-slate-900 border border-slate-700 text-white rounded-lg px-2 py-1 text-xs text-center focus:outline-none focus:ring-1 focus:ring-cyan-500/40" />
                  </div>
                </div>
                <div className="flex items-center gap-3">
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">Incl. eigen goals tegenstanders en spelers buiten selectie</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <label className="text-xs text-slate-500">Punten:</label>
                    <input type="number" min="0" value={predConfigForm.totalGoalsPoints} onChange={(e) => setPredConfigForm(f => ({ ...f, totalGoalsPoints: e.target.value }))}
                      className="w-16 bg-slate-900 border border-slate-700 text-white rounded-lg px-2 py-1 text-xs text-center focus:outline-none focus:ring-1 focus:ring-cyan-500/40" />
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <input type="number" min="0" placeholder="Min" value={predConfigForm.totalGoalsMin} onChange={(e) => setPredConfigForm(f => ({ ...f, totalGoalsMin: e.target.value }))} className={INPUT + " w-24"} />
                  <span className="text-slate-500 text-sm">t/m</span>
                  <input type="number" min="0" placeholder="Max" value={predConfigForm.totalGoalsMax} onChange={(e) => setPredConfigForm(f => ({ ...f, totalGoalsMax: e.target.value }))} className={INPUT + " w-24"} />
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600">Deelnemers zien hoeveel punten elke voorspelling waard is bij het invulscherm</p>
                </div>
              </label>
              ...
              <div className="flex gap-3 flex-wrap items-center border-t border-slate-800 pt-5">
                ...
                {!predConfig?.processed ? (
                  <button onClick={openPredPreview} className="px-4 py-2 bg-emerald-800 hover:bg-emerald-700 text-white rounded-lg font-semibold text-sm transition-colors border border-emerald-500/30">
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 neon-border rounded-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-5">
              ...
              <button onClick={() => setShowPredPreview(false)} className="text-slate-500 hover:text-slate-300 text-xl leading-none">×</button>
            </div>
            ...
            {loadingPredPreview ? (
              ...
              <>
                <p className="text-xs text-slate-500 mb-3">
                  <span className="font-semibold text-white">{predPreview.total}</span> deelnemers hebben voorspellingen ingediend
                </p>
                <div className="space-y-3 mb-5">
                  {[
                    {
                      label: "Topscorer",
                      answer: predPreview.config.topScorer?.name ?? <span className="text-slate-600 italic">Niet ingesteld</span>,
                      points: predPreview.config.topScorerPoints,
                      count: predPreview.topScorerCount,
                    },
                    {
                      label: "Assistkoning",
                      answer: predPreview.config.assistKoning?.name ?? <span className="text-slate-600 italic">Niet ingesteld</span>,
                      points: predPreview.config.assistKoningPoints,
                      count: predPreview.assistKoningCount,
                    },
                    {
                      label: "Gele kaarten",
                      answer: predPreview.config.yellowCardsMin != null && predPreview.config.yellowCardsMax != null
                    ...
                    },
                    {
                      ...
                      answer: predPreview.config.totalGoalsMin != null && predPreview.config.totalGoalsMax != null
                    ...
                    <div key={row.label} className="flex items-center justify-between bg-slate-800/60 rounded-xl px-4 py-3 border border-slate-700/50 gap-3">
                      <div className="min-w-0">
                        <p className="text-xs text-slate-500 font-semibold uppercase tracking-wide">{row.label}</p>
                ...
                <div className="flex gap-3">
                  <button onClick={() => setShowPredPreview(false)} className={BTN_SECONDARY}>Annuleer</button>
                  <button onClick={processBonusPoints} disabled={predProcessing} className="flex-1 px-4 py-2 bg-emerald-700 hover:bg-emerald-600 text-white rounded-lg font-semibold text-sm transition-colors disabled:opacity-50">
                  ...
                  {pred ? (
                    <div className="space-y-1.5 mb-4">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-white font-medium">{pred.totalGoals ?? <span className="text-slate-600 italic">Niet ingevuld</span>}</span>
                      </div>
                    </div>
                  ) : (
          ...
          <div className="bg-slate-900 rounded-2xl w-full max-w-lg border border-slate-700 shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700 shrink-0">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Vink aan welke wedstrijd meetelt voor deze spelers</p>
              </div>
              <button onClick={() => setConflictModal(null)} className="text-slate-500 hover:text-slate-300 text-xl leading-none">×</button>
            </div>
                ...
                return (
                  <div key={conflict.playerId} className="rounded-xl border border-slate-700 bg-slate-800/50 p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs px-1.5 py-0.5 rounded bg-violet-900/50 text-violet-300 border border-violet-500/30 font-medium">FLEX</span>
                      <span className="text-xs text-slate-500">{POSITION_LABEL[conflict.player.position] ?? conflict.player.position}</span>
                    </div>
                    <p className="text-xs text-slate-400 mb-2">Oorspronkelijk elftal: <span className="text-slate-300">{TEAM_LABEL[conflict.player.clubTeam] ?? conflict.player.clubTeam}</span></p>
                    <div className="space-y-2">
                      {conflict.matches.map((m) => {
                        const isChecked = selected.has(m.matchId);
                        return (
                          <label key={m.matchId} className={`flex items-start gap-3 rounded-lg border p-3 cursor-pointer transition-colors ${isChecked ? "border-cyan-500/50 bg-cyan-900/20" : "border-slate-600 bg-slate-800 hover:border-slate-500"}`}>
                            <input
                              type="checkbox"
                              checked={isChecked}
                              onChange={() => {
                                setConflictModal((prev) => {
                                  ...
                                  return { ...prev, selections: { ...prev.selections, [conflict.playerId]: next } };
                                });
                              }}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                {m.isOriginalTeam && (
                                  <span className="text-xs px-1.5 py-0.5 rounded bg-green-900/40 text-green-400 border border-green-500/20 font-medium">Oorspronkelijk</span>
                                )}
                              </div>
                              <span className="text-xs text-slate-400">{TEAM_LABEL[m.matchClubTeam] ?? m.matchClubTeam}</span>
                            </div>
                          </label>
                        );
                      })}
                    ...
                    {noneSelected && (
                      <p className="text-xs text-amber-400 mt-2">Selecteer minimaal één wedstrijd om te verwerken, of deselecteer alles om de speler over te slaan.</p>
                    )}
                  </div>
                );
              })}
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-700 shrink-0">
              <button onClick={() => setConflictModal(null)} className={BTN_SECONDARY}>Annuleer</button>
              <button
                onClick={confirmPublishWithConflicts}
