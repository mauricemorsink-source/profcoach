...
export async function GET() {
  ...
  if (!config) return NextResponse.json({ showPointsToParticipants: false, topScorerPoints: 5, assistKoningPoints: 5, yellowCardsPoints: 5, totalGoalsPoints: 5 });
  return NextResponse.json({
    ...
    totalGoalsPoints: config.totalGoalsPoints,
  });
}
